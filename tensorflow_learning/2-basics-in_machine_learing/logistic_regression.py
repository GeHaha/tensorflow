# -*- coding: utf-8 -*-
"""
Created on Wed Feb  6 22:04:23 2019
...
tf.app.flags.DEFINE_string('max_num_checkpoint',10,
                           'Maximum number if checkpoints that Tensorflow will keep.')
...
tf.app.flags.DEFINE_float('initial_learinig_rate',0.001,'Initial learning rate')
...
tf.app.flags.DEFINE_boolean('is_trainiing',False,
                            'Training/Testing.')

tf.app.flags.DEFINE_boolean('fine_tuning',False,
                            'Fine tuning is desired or not.')

tf.app.flags.DEFINE_boolean('allow_soft__placement',True,
                            'Automatically put the variables on CPU if there is not GPU support.')
...
if not os.path.isabs(FLAGS.train_path):
    raise ValueError('You must aggign absolute path for --train_path')
...
# get only the samples with zero and one label fpr training
